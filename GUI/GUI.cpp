...
void GUI::CreateMenu() const
{
	...
	MenuItemImages[ITM_STU_LEVEL]  = "GUI\\Images\\Menu\\student_level.jpg";
	...
	MenuItemImages[ITM_COURSE_STATUS] = "GUI\\Images\\Menu\\status.jpg";
	MenuItemImages[ITM_LOAD_PLAN] = "GUI\\Images\\Menu\\Menu_load.jpg";
...
{  
	...
	else if(pCrs->getCourseType()==UNIVE)
			pWind->SetPen(UNIVEcolor, 2);
		else if(pCrs->getCourseType() == UNIVC)
			pWind->SetPen(UNIVCcolor, 2);
		else if (pCrs->getCourseType() == TRACKE)
			pWind->SetPen(TRACKEcolor, 2);
		else if (pCrs->getCourseType() == TRACKC)
			pWind->SetPen(TRACKCcolor, 2);
		else if (pCrs->getCourseType() == CONC || pCrs->getCourseType() == CONE)
			pWind->SetPen(CONCcolor, 2);
		else if (pCrs->getCourseType() == MAJORC)
			pWind->SetPen(MAJORCcolor, 2);
		else if (pCrs->getCourseType() == MAJORE)
			pWind->SetPen(MAJOREcolor, 2);
		else if (pCrs->getCourseType() == MINOR)
			pWind->SetPen(MINORcolor, 2);
	
	
	graphicsInfo gInfo = pCrs->getGfxInfo();
	if (pCrs->getCourseType() == TRACKE || pCrs->getCourseType() == MAJORE || pCrs->getCourseType() == UNIVE || pCrs->getCourseType() == CONE) {
		pWind->DrawRectangle(gInfo.x, gInfo.y, gInfo.x + CRS_WIDTH, gInfo.y + CRS_HEIGHT,FILLED,4,4);
	}
	else {
		...
		while (true)
		{
			...
			if (ctInput == LEFT_CLICK)	//mouse left click
			{
				...
				if (y >= 0 && y < MenuBarHeight)
				{
					...
					case ITM_COURSE_STATUS: return ActionData{ COURSE_STATUS };
					case ITM_LOAD_PLAN:return ActionData{ LOAD };
					case ITM_STU_LEVEL: return ActionData{ STU_LEVEL };
					case ITM_MINOR: return ActionData{ ADD_MINOR };		//Exit
					break;
...
void GUI::OpenCheckWindow() {
	pValidityWindow = new window(450, 600, 500, 100);
	pValidityWindow->ChangeTitle("Check Report");
	pValidityWindow->SetBrush(ROYALBLUE);
...
void GUI::Info_Window(Course* P_course)
{
	...
	if (P_course != nullptr)
	{
		...
		p_Infowind->SetPen(infoColor);
		if (prereq.empty()) {
			p_Infowind->DrawString(0, msg_y, "There are no Prerequisite Courses for this course");
			msg_y += 15;
		}
		else {
...
void GUI::DrawString(string s, int x, int y) {
	pWind->SetFont(20, 2, ROMAN);
	pValidityWindow->SetPen(BLACK, 2);
	pValidityWindow->DrawString(x, y, s);
